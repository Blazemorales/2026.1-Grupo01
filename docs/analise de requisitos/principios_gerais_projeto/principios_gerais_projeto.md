...
Os princípios gerais de projeto em IHC constituem um conjunto de diretrizes de alto nível que orientam a criação de sistemas cujo modelo conceitual seja facilmente assimilado pelo usuário.
O objetivo central desses princípios é reduzir a discrepância entre os objetivos psicológicos das pessoas e os mecanismos físicos da interface, simplificando a travessia dos golfos de execução e avaliação.
...
Para este artefato os alunos separam os principios de projeto descritos no livro, assim cada aluno tem no minimo um principio para avaliar no site, ele deve descrever como site executa esse principio, se executar, e se o executa de forma satisfatória. 
